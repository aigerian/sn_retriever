...
1) Не работает визуализация (https://github.com/AlexeyProskuryakov/sn_retriever/issues/9)
Предлагаю включить какой-нибудь teamviewer чтобы посмотреть какая была ошибка с визуализацией. У меня почему-то все работает

2) Виснет из-за потоков нужно переделать, чтобы было более стабильно.
