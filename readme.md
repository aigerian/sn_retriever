...
*Message* сообщения пользователей. Из twitter это сосбтвенно сообщений которые написал пользователь, из vk - это комментарии которые которые оставли пользователи. Общие поля таковы: 
	1) source - собственно имя ресурса (ttr - твиттер, vk - вконтакте, fb - фейсбук)
	2) text - текст сообщения
	3) created_at - время создания сообщения 
	4) sn_id - идентиикатор сообщения в социальной сети (для vk состоит из строчки характеризующее этот комментарий)
	5) user - ссылка на пользователя 

*SocialObject* Характеризует группу пользователей: некоторый объект который имеет свой контент (список обсуждений, фотографии, видео, и прочее). 
