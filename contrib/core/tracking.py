...
class TTR_Tracking(object):
    ...
    def get_relations_diff(self, user, delta, relation_type='friends', update=True):
        saved_relations = self.db.retrieve_relations_for_diff(user.get('_id'), relation_type=relation_type)
        new = []
        remove = []
        acc = []
        from_cursor = -1
        ...
        while (updated_delta != 0):
            result = self.api.get_relation_ids(user, relation_type, from_cursor)
            updated_delta = delta
            if result:
                batch, cursor = result
        ...
        if update:
            self.db.save_relations_for_diff(user.get('_id'), acc, relation_type)
